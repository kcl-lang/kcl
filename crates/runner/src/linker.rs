...
const KCL_CLI_BIN_PATH_ENV_VAR: &str = "KCL_CLI_BIN_PATH";
const KCL_LIB_LINK_PATH_ENV_VAR: &str = "KCL_LIB_LINK_PATH";
const KCL_LIB_SHORT_NAME: &str = "kcl";
const EXEC_ROOT_NOT_FOUND_MSG: &str = "Internal error: the executable root is not found";

/// KclLinker is mainly responsible for linking the libs generated by KclAssembler.
pub struct KclLinker;
impl KclLinker {
...
impl Command {
    ...
    pub(crate) fn unix_args(
        ...
        cmd.args(libs)
            ...
            .arg(&format!("-l{KCL_LIB_SHORT_NAME}"));
        Ok(())
    }
    ...
    pub(crate) fn msvc_win_args(
        ...
        cmd.args(libs)
            .arg(&format!("{KCL_LIB_SHORT_NAME}.lib"))
            .arg("/link")
            .arg("/NOENTRY")
            .arg("/NOLOGO")
            ...
            .arg("/EXPORT:_kcl_run")
            .arg("/EXPORT:kcl_main")
            .arg("/EXPORT:kcl_plugin_init");
        Ok(())
    }

    /// Get the executable root.
    fn get_executable_root() -> Result<String> {
        if let Ok(path) = std::env::var(KCL_CLI_BIN_PATH_ENV_VAR) {
            return Ok(path);
        }
    ...
    pub(crate) fn get_lib_link_path(&self) -> Result<String> {
        ...
        Ok(std::env::var(KCL_LIB_LINK_PATH_ENV_VAR)
            .ok()
            .or(default_path)
    ...
    pub(crate) fn get_lib_name() -> String {
        ...
        if Self::is_windows() {
            format!("{KCL_LIB_SHORT_NAME}{suffix}")
        } else {
            format!("lib{KCL_LIB_SHORT_NAME}{suffix}")
        }
    }
