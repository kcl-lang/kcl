...
/// `read()` is a thin wrapper around `fs::read_to_string` and fails on
/// non-UTF-8 input, so it cannot be used for binary assets (images,
/// certificates, archives, …). `readbase64()` accepts any byte sequence
/// and round-trips it losslessly through the standard alphabet
