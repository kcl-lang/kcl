...
impl DiagnosticHandler {
    ...
    /// ```
    pub fn add_err_diagnostic(&mut self, diag: Diagnostic<DiagnosticStyle>) {
    ...
    /// assert_eq!(diag_handler.diagnostics_count(), 1);
    /// ```
    pub fn add_warn_diagnostic(&mut self, diag: Diagnostic<DiagnosticStyle>) {
        self.diagnostics.push(diag);
        self.warn_count += 1;
    }
    ...
    pub fn emit_error_diagnostic(&mut self, diag: Diagnostic<DiagnosticStyle>) {
        self.emitter.emit_diagnostic(&diag);
        self.err_count += 1;
    }

    /// Emit the diagnostic messages generated from warning to to terminal stderr.
    pub fn emit_warn_diagnostic(&mut self, diag: Diagnostic<DiagnosticStyle>) {
        self.emitter.emit_diagnostic(&diag);
        self.warn_count += 1;
    }
    ...
    /// `DiagnosticHandler` contains a set of `Diagnostic<DiagnosticStyle>`
    pub fn emit_stashed_diagnostics(&mut self) {
