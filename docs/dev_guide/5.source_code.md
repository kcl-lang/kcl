...
+ `crates` contains the source code for the KCL core. It consists of many crates that together make up the compiler.
...
+ `cmd` houses the command-line interface tools, enabling the users to interact with the compiler via the command-line such as `libkcl` binary.
