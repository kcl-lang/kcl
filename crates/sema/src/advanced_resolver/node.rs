...
impl<'ctx> MutSelfTypedResultWalker<'ctx> for AdvancedResolver<'_> {
    ...
    fn walk_unification_stmt(
        ...
        if let Some(parent_scope) = self.ctx.scopes.last()
            && let Some(parent_scope) = self.gs.get_scopes().get_scope(parent_scope)
        {
            let mut doc = None;
            if let Some(schema_symbol) = parent_scope.get_owner() {
                ...
                if let Some(schema_ty) = schema_symbol.get_sema_info().ty.clone()
                    && !unification_stmt.target.node.names.is_empty()
                {
        ...
        for target in quant_expr.variables.iter() {
            ...
            let ast_id = if target.node.names.is_empty() {
                ...
                &target
                    ...
                    .names
                    .last()
                    .ok_or_else(|| anyhow::anyhow!("empty target names"))?
                    .id
            };
            let value = self.gs.get_symbols_mut().alloc_value_symbol(
    ...
    fn walk_schema_attr(&mut self, schema_attr: &'ctx ast::SchemaAttr) -> Self::Result {
        ...
        let parent_scope = self.ctx.get_current_scope()?;
        let parent_scope = self
            .gs
            .get_scopes()
            .get_scope(&parent_scope)
...
impl<'ctx> AdvancedResolver<'_> {
    ...
    fn resolve_names(&mut self, names: &[ast::Node<String>], maybe_def: bool) -> ResolvedResult {
        let first_name = names
            .first()
            .ok_or_else(|| anyhow::anyhow!("empty names"))?;
        let cur_scope = self.ctx.get_current_scope()?;
        ...
        if first_symbol.is_none() {
            ...
            let module_info = self
                ...
                .ok_or_else(|| anyhow::anyhow!("module info not found"))?;
            ...
            Some(symbol_ref) => {
                ...
                if names.len() > 1 {
                    ...
                    for index in 1..names.len() {
                        let name = names
                            .get(index)
                            .ok_or_else(|| anyhow::anyhow!("name index out of bounds"))?;
                        let def_symbol_ref = match self.gs.get_symbols().get_type_attribute(
                            &parent_ty,
                            &name.node,
                            self.get_current_module_info(),
    ...
    pub fn walk_identifier_expr_with_hint(
            ...
            .copied()
        {
            ...
            if let Some(symbol) = symbols.values.get_mut(identifier_symbol.get_id()) {
                let id = if identifier.node.names.is_empty() {
                    ...
                    &identifier
                        ...
                        .names
                        .last()
                        .ok_or_else(|| anyhow::anyhow!("empty identifier names"))?
                        .id
                };
                let ty = self
                    .ctx
            ...
            if self.ctx.maybe_def && !identifier.node.names.is_empty() {
                let cur_scope = self.ctx.get_current_scope()?;
                match cur_scope.kind {
                    crate::core::scope::ScopeKind::Local => {
                        self.gs.get_scopes_mut().add_def_to_scope(
                        ...
                        Some(arg_ref) => match arg_ref.get_kind() {
                            crate::core::symbol::SymbolKind::Unresolved => {
                                ...
                                if let Some(unresolved) =
                                    ...
                                    && let Some(def) = unresolved.def
                                    && let Some(def) = symbol_data.get_symbol(def)
                                    && def.get_name() != param.name
                                {
                                    has_hint = true;
                                }
                                if has_hint {
        ...
        for entry in entries.iter() {
            ...
            if let Some(key) = &entry.node.key {
                ...
                if let Some(symbol_ref) = self.expr(key)?
                    && let Some(config_key_symbol) =
                        self.gs.get_symbols().unresolved.get(symbol_ref.get_id())
                    && let Some(def_ref) = config_key_symbol.get_definition()
                    && let Some(def_symbol) = self.gs.get_symbols().get_symbol(def_ref)
                {
