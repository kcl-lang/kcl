...
use kclvm_sema::resolver::pos::GetPos;
...
impl<'a> Parser<'_> {
    ...
    pub(crate) fn parse_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        // skip new lines
    ...
    /// Syntax:
    /// simple_stmt: (assign_stmt | expr_stmt | assert_stmt | import_stmt | type_alias_stmt) NEWLINE
    fn parse_compound_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        // skip new lines
        if matches!(self.token.kind, TokenKind::Newline) {
            self.skip_newlines();
        }
    ...
    fn parse_simple_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        // skip new lines
        if matches!(self.token.kind, TokenKind::Newline) {
            self.skip_newlines();
        }
    ...
    pub(crate) fn parse_block_stmt_list(
        ...
        open_tok: TokenKind,
        close_tok: TokenKind,
    ) -> Result<Vec<Box<Node<Stmt>>>, Diagnostic> {
    ...
    fn parse_expr_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
    ...
    ///     test
    fn parse_expr_or_assign_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        let token = self.token;
        ...
        if let Some(value) = value_or_target {
            ...
            let mut targets_tmp = vec![];
            for expr in targets {
                match &expr.node {
                    Expr::Identifier(x) => {
                        let mut x = x.clone();
                        x.ctx = ExprContext::Store;
                        targets_tmp.push(Box::new(Node::node_with_pos(x, expr.pos())));
                    }
                    _ => {
                        return Err(self
                            .sess
                            .struct_compile_error(&format!("invalid target"), expr.get_pos()));
                    }
                }
            }
            let targets = targets_tmp;
    ...
    fn parse_assert_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        let token = self.token;
    ...
    fn parse_import_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        let token = self.token;
    ...
    fn parse_type_alias_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
    ...
    fn parse_if_stmt(&mut self) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        let token = self.token;
    ...
    ) -> Result<Option<NodeRef<Stmt>>, Diagnostic> {
        let token = self.token;
    ...
    fn parse_decorators(&mut self) -> Result<Vec<NodeRef<CallExpr>>, Diagnostic> {
        ...
        while let TokenKind::At = self.token.kind {
            ...
            match expr.node {
                ...
                _ => {
                    self.sess
                        .struct_compile_error("Invalid Decorator", expr.get_pos());
                }
            };
    ...
    pub(crate) fn parse_parameters(
        ...
        bump_close: bool,
    ) -> Result<Option<NodeRef<Arguments>>, Diagnostic> {
    ...
    fn parse_schema_body(&mut self) -> Result<SchemaStmt, Diagnostic> {
    ...
    fn parse_mixins(&mut self) -> Result<Vec<NodeRef<Identifier>>, Diagnostic> {
        self.bump_keyword(kw::Mixin);
    ...
    fn parse_schema_attribute(&mut self) -> Result<SchemaAttr, Diagnostic> {
        let doc = "".to_string();
    ...
    fn parse_schema_index_signature_or_list(
        &mut self,
    ) -> Result<(Option<SchemaIndexSignature>, Option<ListExpr>), Diagnostic> {
    ...
    fn parse_schema_check_block(&mut self) -> Result<Vec<NodeRef<CheckExpr>>, Diagnostic> {
    ...
    ) -> Result<Option<JoinedString>, Diagnostic> {
        // skip raw string
        ...
        fn parse_expr(
            ...
            if let TokenKind::Colon = parser.token.kind {
                ...
                if let TokenKind::DocComment(_) = parser.token.kind {
                    ...
                    return Err(this
                        .sess
                        .struct_span_error("invalid joined string spec", parser.token.span));
                }
            }

            Ok(node_ref!(Expr::FormattedValue(formatted_value)))
        }
