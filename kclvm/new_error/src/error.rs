...
use crate::{
    ...
    pendant::{CodeCtxPendant, HeaderPendant, LabelPendant},
...
const A:() = {impl DiagnosticBuilder for ThisIsAnErr {
    fn into_diagnostic(self) -> Diagnostic {
        let mut diagnostic = Diagnostic::new();
        match self
            {
                ThisIsAnErr { pos : ref __binding_0, } =>
                {
                    {
                        let codectx_pendant = CodeCtxPendant ::
                        new(self.pos.clone()) ; let codectx_sentence = Sentence ::
                        new_sentence_str(Box :: new(codectx_pendant), Message ::
                        Str("oh no! this is an error!".to_string())) ;
        ...
        let title_sentence = Sentence::new_sentence_str(
            ...
            Message::Str("oh no! this is an error!".to_string()),
        );
        // let codectx_sentence = Sentence::new_sentence_str(
        //     Box::new(codectx_pendant),
        //     Message::Str("help: try using a qux here: `qux sad()`".to_string()),
        // );
        let label_sentence = Sentence::new_sentence_str(
            Box::new(label_pendant),
            Message::Str("oh no! this is an error!".to_string()),
        );
        let nopendant_sentence = Sentence::new_nopendant_sentence(Message::Str(
            "For more information about this error, try `rustc --explain E0999`.".to_string(),
        ));
